settings = {
    ...
    'MAPS' : {
      ...
      'queries' : {
        'project' : 'MAPS',
        'epics' : 'project = MAPS AND issuetype = Epic AND status != Closed AND (labels != nobacklog OR labels is empty) ORDER BY Rank ASC',
        'stories' : 'project = MAPS and issuetype = Story and "Epic Link" in (%s) AND (labels != nobacklog OR labels is empty) ORDER BY Rank ASC',
        'tasks' : 'project = MAPS AND (issuetype in (subTaskIssueTypes(), Bug, Improvement, "Technical task")) AND parent in (%s) AND (labels != nobacklog OR labels is empty) ORDER BY Rank ASC'
      }
    },
    'NDA' : {
      'name' : 'Spotlight',
